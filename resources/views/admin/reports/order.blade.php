...
<div class="p-6 bg-white border rounded-lg">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-primary">{{__('sidebar.Orders')}}</h1>
        </div>

        <div class="flex gap-6 mt-6 ">
            <form action="{{route('filter.order')}}" class="flex" method="get">
            <div>
            ...
            <div>
                <label class="text-gray-700">{{__('order.To')}}</label>
    ...
    <div x-data="{ openModel: false }" class="overflow-x-auto">
        <div class="inline-block min-w-full overflow-hidden align-middle">
            <table class="min-w-full text-sm text-gray-500 lg:text-base" id="orders-table" cellspacing="0">
                <thead>
                    <tr class="h-12">
                        ...
                        <th class="px-6 py-4 {{app()->getLocale() == 'en' ? 'text-left' : 'text-right' }}">
                            {{__('order.Price')}}</th>
                        <th class="px-6 py-4 text-center">{{__('order.Date')}}</th>
                    </tr>
                </thead>
