...
<div :class="sidebarOpen ? 'translate-x-0 ease-out' : 'translate-x-full ease-in'" class="fixed inset-y-0 z-30 right-0 overflow-y-auto transition duration-300 transform bg-white border-l w-60 lg:translate-x-0 lg:static lg:inset-0">
    ...
    <nav class="mt-10">
        ...
        <a class="flex items-center px-6 py-2 mt-4 {{ Request::is('admin/categories*') ? 'text-gray-600 bg-gray-300 bg-opacity-25' : 'text-gray-500 hover:bg-gray-300 hover:bg-opacity-25 hover:text-gray-600' }}" href="{{route('categories.index')}}">
            ...
            <span class="mx-3 font-semibold">{{__('category.Category')}}</span>
        </a>
        ...
        <a class="flex items-center px-6 py-2 mt-4 {{ Request::is('admin/cars*') ? 'text-gray-600 bg-gray-300 bg-opacity-25' : 'text-gray-500 hover:bg-gray-300 hover:bg-opacity-25 hover:text-gray-600' }}" href="{{route('cars.index')}}">
            ...
            <span class="mx-3 font-semibold">{{__('car.Car')}}</span>
        </a>

        <div x-data="{{ Route::is('admin/report*') || Route::is('admin/filter*') ? '{ open: true }' : '{ open: false }'}}" class="mt-4">
            <button @click="open = !open" class="flex items-center justify-between w-full px-6 py-2 text-gray-600 hover:bg-gray-100 hover:text-gray-700 focus:outline-none">
                <span class="flex items-center">
                    ...
                    <span class="mx-4 font-medium">{{__('sidebar.Reports')}}</span>
                </span>
            ...
            <div
                ...
                class="bg-gray-100"
            >
                <a class="block px-16 py-2 text-sm {{ Request::is('admin/report/orders*') || Request::is('admin/filter/orders*') ? 'text-white bg-green-500' : 'text-gray-600 hover:bg-green-500 hover:text-white' }}" href="{{route('report.order')}}">
                    {{__('sidebar.Orders')}}
                </a>
                <a class="block px-16 py-2 text-sm {{ Request::is('admin/report/users*') || Request::is('admin/filter/users*') ? 'text-white bg-green-500' : 'text-gray-600 hover:bg-green-500 hover:text-white' }}" href="{{route('report.user')}}">
                    {{__('sidebar.Users')}}
                </a>
            </div>
        </div>
